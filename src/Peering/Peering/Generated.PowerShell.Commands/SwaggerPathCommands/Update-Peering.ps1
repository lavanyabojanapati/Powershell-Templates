...
#>
function Update-Peering
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.Peering])]
    [CmdletBinding(DefaultParameterSetName='Peerings_Update')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'Peerings_Update')]
        [System.String]
        $PeeringName,
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'Peerings_Update')]
        [Microsoft.PowerShell.AzPeering.v001.Models.ArmTags]
        $Tags
    )
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Peerings_Update' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation UpdateWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.Peerings.UpdateWithHttpMessagesAsync($ResourceGroupName, $PeeringName, $Tags)
    } else {
