...
#>
function Get-PeeringOperation
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.ArmOperation])]
    [CmdletBinding(DefaultParameterSetName='PeeringOperations_List')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'PeeringOperations_List')]
        [int]
        $Skip = -1,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('PeeringOperations_List' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.PeeringOperations.ListWithHttpMessagesAsync()
    } else {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.PowerShell.AzPeering.v001.Models.ArmOperation]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $EdgeRp.PeeringOperations.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
