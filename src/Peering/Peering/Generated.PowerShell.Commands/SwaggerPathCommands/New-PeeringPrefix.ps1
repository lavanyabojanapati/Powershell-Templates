...
#>
function New-PeeringPrefix
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.PeeringPrefix])]
    [CmdletBinding(DefaultParameterSetName='PeeringPrefixes_CreateOrUpdate')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'PeeringPrefixes_CreateOrUpdate')]
        [Microsoft.PowerShell.AzPeering.v001.Models.PeeringPrefix]
        $Prefix,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('PeeringPrefixes_CreateOrUpdate' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation CreateOrUpdateWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.PeeringPrefixes.CreateOrUpdateWithHttpMessagesAsync($ResourceGroupName, $PeeringName, $PrefixName, $Prefix)
    } else {
