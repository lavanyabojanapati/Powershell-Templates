...
#>
function Get-PeeringSubscriber
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.PeeringSubscriber])]
    [CmdletBinding(DefaultParameterSetName='PeeringSubscribers_Get')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'PeeringSubscribers_Get')]
        [System.String]
        $PeeringName,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('PeeringSubscribers_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.PeeringSubscribers.GetWithHttpMessagesAsync($ResourceGroupName, $PeeringName, $SubscriberName)
    } else {
