...
#>
function Get-PeeringAll
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.Peering])]
    [CmdletBinding(DefaultParameterSetName='Peerings_ListAll')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'Peerings_ListAll')]
        [int]
        $Skip = -1,
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('Peerings_ListAll' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListAllWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.Peerings.ListAllWithHttpMessagesAsync()
    } else {
    ...
    if ($TaskResult) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $EdgeRp.Peerings.ListAllNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
