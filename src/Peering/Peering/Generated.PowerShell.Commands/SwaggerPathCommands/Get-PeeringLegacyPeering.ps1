...
#>
function Get-PeeringLegacyPeering
{
    [OutputType([Microsoft.PowerShell.AzPeering.v001.Models.Peering])]
    [CmdletBinding(DefaultParameterSetName='PeeringLegacyPeerings_List')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'PeeringLegacyPeerings_List')]
        [int]
        $Skip = -1,
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'PeeringLegacyPeerings_List')]
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('PeeringLegacyPeerings_List' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation ListWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.PeeringLegacyPeerings.ListWithHttpMessagesAsync($Kind)
    } else {
    ...
    if ($TaskResult) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $EdgeRp.PeeringLegacyPeerings.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
