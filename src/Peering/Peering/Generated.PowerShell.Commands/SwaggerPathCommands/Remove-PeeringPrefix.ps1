...
#>
function Remove-PeeringPrefix
{
    [CmdletBinding(DefaultParameterSetName='PeeringPrefixes_Delete')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'PeeringPrefixes_Delete')]
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('PeeringPrefixes_Delete' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation DeleteWithHttpMessagesAsync on $EdgeRp.'
        $taskResult = $EdgeRp.PeeringPrefixes.DeleteWithHttpMessagesAsync($ResourceGroupName, $PeeringName, $PrefixName, $Prefix)
    } else {
